...
export const aboutSectionType = defineType({
  ...
  preview: {
    ...
    prepare({ title, layout, image, stats }) {
      ...
      const titleText = title?.[0]?.children?.map((child: any) => child.text).join('') || 'About Section';
      const statsCount = stats?.length || 0;
      const layoutText = layout === 'textOnly' ? 'Text Only' : 
                        layout === 'textWithImage' ? 'Text + Image' : 'Two Columns';
